    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
          ...
          <span className={`text-xs px-2.5 py-1 rounded border ${statusColor(meeting.status)}`}>
          ...
          <ul className="space-y-2 text-sm">
            {procedural.map((r) => (
              <li key={r.id} className="text-gray-700">
                ...
                {r.decision_text && <span className="text-gray-500"> – {r.decision_text}</span>}
              </li>
            ))}
          </ul>
        </div>
      ...
      <h2 className="text-lg font-semibold text-gray-900">Darbotvarkė</h2>
...
function ResolutionCard({
  ...
  const totalVotes = resolution.result_for + resolution.result_against + resolution.result_abstain;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {resolution.description && (
            <p className="text-sm text-gray-600 mt-1 whitespace-pre-wrap">{resolution.description}</p>
          )}
        </div>
      </div>
