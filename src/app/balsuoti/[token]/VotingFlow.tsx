...
interface Props {
  token: string;
  data: {
    meeting: { id: string; title: string; description: string | null; meeting_date: string; location: string };
    member: { id: string; first_name: string; last_name: string; email: string | null; phone: string | null };
    resolutions: Resolution[];
    expires_at: string;
  };
}

type Step = "contacts" | "voting" | "review" | "done";
    ...
    <div className="flex items-center gap-2">
      {steps.map((s, i) => (
        <div key={s.key} className="flex items-center flex-1">
          ...
          {i < steps.length - 1 && (
            <div
              className={`flex-1 h-0.5 mx-3 ${i < currentIdx ? "bg-blue-600" : "bg-gray-200"}`}
            />
          )}
        </div>
      ))}
...
function MeetingHeader({ meeting }: { meeting: Props["data"]["meeting"] }) {
  return (
        ...
        <div key={r.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(["uz", "pries", "susilaike"] as VoteChoice[]).map((choice) => {
              ...
              const colors = {
                uz: selected ? "bg-green-600 text-white border-green-600" : "border-gray-300 text-gray-700 hover:bg-green-50 hover:border-green-300",
                pries: selected ? "bg-red-600 text-white border-red-600" : "border-gray-300 text-gray-700 hover:bg-red-50 hover:border-red-300",
                susilaike: selected ? "bg-gray-600 text-white border-gray-600" : "border-gray-300 text-gray-700 hover:bg-gray-50",
              };
              return (
                <button
                  key={choice}
